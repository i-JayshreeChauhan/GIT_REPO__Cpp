...
After a few hours, they came up with some promising source code. Unfortunately, 
it doesn't compile! Since you don't want to discourage them, you decide to make their code work without modifying it
 by adding preprocessor macros.
...
You are not responsible for printing any output to stdout. Once the necessary preprocessor macros are written, 
the locked code in your editor will print a line that says RESULT=Z, where Z is the difference between the maximum and minimum values in the array.
...
// Defines a macro function for min and max comparison
#define FUNCTION(name, op) \
    void name(int &a, int b) { if (b op a) a = b; }

// Large constant for initializing min and max
#define INF 10000000

// Loops through each element in a vector
